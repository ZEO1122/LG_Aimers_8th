...
"""
score_zip.py (server-aligned, prompt-agnostic speed)

- ZIP 제출물(압축/양자화 모델)을 입력으로 받아
  1) lm_eval로 Perf_model, Perf_base 평가 (hf 또는 vllm backend)
  2) vLLM로 speed(TimePerToken) 측정 (프롬프트 미제공 상황을 고려해 GSM8K 질문 기반)
...
- speed 프롬프트가 제공되지 않는 경우:
  -> gsm8k 질문을 샘플링해 프롬프트를 구성(가능하면 datasets), 실패 시 fallback 프롬프트 사용
- speed 측정이 과대평가되지 않도록 기본적으로 vLLM 엔진 초기화 시간 포함
...
def find_hf_model_dir(root: Path) -> Path:
    """ZIP 안에서 HuggingFace 모델 폴더(config.json 존재)를 찾는다."""
...
def find_latest_results_json(output_root: Path) -> Path:
    """lm_eval --output_path가 디렉토리일 때 생성되는 results_*.json 중 최신 파일 반환."""
...
def run_lm_eval(
    ...
    if backend == "hf":
        ...
        # -> model_args에 넣지 말고 --apply_chat_template 플래그로 켜야 함
        model_args = (
...
def extract_perf_from_lm_eval(
    ...
    """
    metric_mode:
      - "strict": exact_match,strict-match 우선
      - "flexible": exact_match,flexible-extract 우선
    """
...
def base_cache_signature(args: argparse.Namespace, base_model_dir: Path) -> Dict[str, Any]:
    return {
        ...
        "limit": args.limit,
        "perf_metric_mode": args.perf_metric_mode,
        ...
        "speed_include_init_time": bool(args.speed_include_init_time),
        "speed_prompt_source": args.speed_prompt_source,
    }
...
def load_gsm8k_questions(n: int) -> List[str]:
    """
    가능한 경우 datasets로 gsm8k test 질문을 가져옴.
    인터넷/캐시 상황에 따라 실패할 수 있으므로 예외 처리.
    """
    from datasets import load_dataset  # type: ignore

    ds = load_dataset("gsm8k", "main", split="test")  # 보통 HF에서 받아옴
    questions = []
    for ex in ds:
        q = ex.get("question")
...
def get_speed_prompts(n: int, source: str) -> List[str]:
    """
    source:
      - "gsm8k": gsm8k test 질문 기반(권장)
      - "fallback": 내장 짧은 프롬프트(최후 수단)
    """
    if source == "gsm8k":
        try:
...
def speed_benchmark_vllm_time_per_token(
    ...
    """
    vLLM로 prompts를 generate하여 time-per-token 측정.
    - include_init_time=True면 LLM 엔진 생성(초기화) 시간까지 포함하여 보수적으로 측정.
    """
...
def main():
    ...
    # Perf metric mode (서버와 다를 수 있는 부분을 보수적으로)
    ...
    # speed benchmark settings (프롬프트 미제공 전제)
    ...
    ap.add_argument("--speed_max_tokens", default=2048, type=int)  # 4096~16384도 가능(시간 증가)
    ap.add_argument("--speed_include_init_time", default=True, action=argparse.BooleanOptionalAction)
